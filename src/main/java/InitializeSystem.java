...
public class InitializeSystem {
  ...
  protected static Context readInput(String args[]) {
    ...
    List<Train> trains =
        initalizeTrains(Arrays.copyOfRange(args, argumentPointer + POINTER_OFFSET, args.length));
  ...
  protected static Graph getMapForRouting(Context ctx) {
    Graph g = new Graph();
    ...
    while (stationIterator.hasNext()) {
      ...
      Node node = nodes.get(station.getName());
      if (node == null) {
        node = new Node(station.getName());
        nodes.put(station.getName(), node);
      }
